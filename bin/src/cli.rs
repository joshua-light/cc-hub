...
/// Release the merge lock first, then flip the PR and task to terminal states.
/// If the release fails the task stays in `Merging` so a re-run can complete the
/// transition — otherwise a transient FS error would strand a `Done` task as
/// the lock holder and block the project's merge queue until `STALE_TTL_SECS`.
