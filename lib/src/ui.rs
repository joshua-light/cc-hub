    ...
    if let Some(msg) = fresh_status {
        ...
        let keybinds: &str = match app.view {
            View::Grid => match app.current_tab {
                Tab::Projects => "tab:next  H/L:project  h/l:col  j/k:task  enter:focus orch  f:agent terminal/resurrect  n:new task  N:register project  b:backlog  r:result  x:delete task  X:remove project  q:quit",
