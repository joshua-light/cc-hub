    ...
    if let Some(msg) = fresh_status {
        ...
        let keybinds: &str = match app.view {
            View::Grid => match app.current_tab {
                Tab::Projects => "tab:next  h/l:column  j/k:task  H/L:project  space:approve review  enter:focus orch  f:agent terminal  n:new task  N:register  r:result  x:delete  q:quit",
...
fn agent_dot_strip(t: &crate::orchestrator::TaskState, sum: &AgentSummary) -> Vec<Span<'static>> {
...
fn render_task_card_active(
    ...
    let title_id = short_task_id(&t.task_id);
    ...
    row2.extend(agent_dot_strip(t, &sum));
    row2.push(Span::raw("   "));
...
/// Trim a task id like `t-1777291032362100387` down to a short, stable
/// suffix suitable for card titles.
fn short_task_id(id: &str) -> String {
    let cleaned = id.strip_prefix("t-").unwrap_or(id);
    let n = cleaned.chars().count();
    if n <= 8 {
        cleaned.to_string()
    } else {
        cleaned.chars().skip(n - 8).collect()
    }
}
